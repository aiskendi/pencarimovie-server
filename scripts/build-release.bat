...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $root='%ROOT2%'; $dist=$root+'\dist'; foreach($target in @('frankenphp-linux-aarch64','frankenphp-linux-aarch64-gnu')){$source=$root+'\'+$target; if(-not (Test-Path $source)){Write-Warning('Source not found: '+$source);continue}; $pkg=$target.Substring(11); $build=$dist+'\.build-tmp\pencarimovie-downloader-'+$pkg; Remove-Item -Recurse -Force $build -ErrorAction SilentlyContinue; New-Item -ItemType Directory -Force -Path ($build+'\bin')|Out-Null; foreach($f in @('Caddyfile','backend.php','index.php','router.php','install.sh','install-termux.sh','start.sh','start-termux.sh','restart.sh','restart-termux.sh','stop.sh','pencarimovie-linux.sh','pencarimovie-termux.sh','warmup-ipc.php','package.json','README.md','LICENSE','SECURITY.md','patches')){$src=$root+'\'+$f;if(Test-Path $src){Copy-Item -Recurse -Force $src $build}}; Copy-Item -Recurse -Force ($root+'\public') ($build+'\public'); New-Item -ItemType Directory -Force -Path ($build+'\storage')|Out-Null; Copy-Item -Force ($root+'\storage\.gitkeep') ($build+'\storage\.gitkeep') -ErrorAction SilentlyContinue; if(Test-Path ($root+'\storage\config.example.json')){Copy-Item -Force ($root+'\storage\config.example.json') ($build+'\storage\config.example.json')}; if(Test-Path ($root+'\storage\catalog_settings.json')){Copy-Item -Force ($root+'\storage\catalog_settings.json') ($build+'\storage\catalog_settings.json')}; Copy-Item -Force ($root+'\bin\php.ini.unix') ($build+'\bin\php.ini'); Copy-Item -Force ($root+'\bin\php') ($build+'\bin\php'); Copy-Item -Force $source ($build+'\bin\frankenphp'); if(Test-Path ($root+'\vendor\autoload.php')){Copy-Item -Recurse -Force ($root+'\vendor') ($build+'\vendor')}; if(Test-Path ($root+'\.release-tag')){Copy-Item -Force ($root+'\.release-tag') ($build+'\.release-tag')}; Get-ChildItem -Path $build -Filter *.sh -Recurse | ForEach-Object { $c = [System.IO.File]::ReadAllText($_.FullName) -replace \"`r\", ''; [System.IO.File]::WriteAllText($_.FullName, $c, (New-Object System.Text.UTF8Encoding $false)) }; $bp = Join-Path $build 'bin\php'; if (Test-Path $bp) { $c = [System.IO.File]::ReadAllText($bp) -replace \"`r\", ''; [System.IO.File]::WriteAllText($bp, $c, (New-Object System.Text.UTF8Encoding $false)) }; $tar=$dist+'\pencarimovie-downloader-'+$pkg+'.tar.gz'; tar -czf $tar -C ($dist+'\.build-tmp') ('pencarimovie-downloader-'+$pkg); Remove-Item -Recurse -Force $build}"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $root='%ROOT2%'; $dist=$root+'\dist'; $build=$dist+'\.build-tmp\pencarimovie-server'; Remove-Item -Recurse -Force $build -ErrorAction SilentlyContinue; New-Item -ItemType Directory -Force -Path ($build+'\bin')|Out-Null; foreach($f in @('Caddyfile','backend.php','index.php','router.php','install.sh','install-termux.sh','start.sh','start-termux.sh','restart.sh','restart-termux.sh','stop.sh','pencarimovie-linux.sh','pencarimovie-termux.sh','warmup-ipc.php','package.json','README.md','LICENSE','SECURITY.md','patches')){$src=$root+'\'+$f;if(Test-Path $src){Copy-Item -Recurse -Force $src $build}}; Copy-Item -Recurse -Force ($root+'\public') ($build+'\public'); New-Item -ItemType Directory -Force -Path ($build+'\storage')|Out-Null; Copy-Item -Force ($root+'\storage\.gitkeep') ($build+'\storage\.gitkeep') -ErrorAction SilentlyContinue; if(Test-Path ($root+'\storage\config.example.json')){Copy-Item -Force ($root+'\storage\config.example.json') ($build+'\storage\config.example.json')}; if(Test-Path ($root+'\storage\catalog_settings.json')){Copy-Item -Force ($root+'\storage\catalog_settings.json') ($build+'\storage\catalog_settings.json')}; Copy-Item -Force ($root+'\bin\php.ini.unix') ($build+'\bin\php.ini') -ErrorAction SilentlyContinue; Copy-Item -Force ($root+'\bin\php') ($build+'\bin\php') -ErrorAction SilentlyContinue; if(Test-Path ($root+'\vendor\autoload.php')){Copy-Item -Recurse -Force ($root+'\vendor') ($build+'\vendor')}; if(Test-Path ($root+'\.release-tag')){Copy-Item -Force ($root+'\.release-tag') ($build+'\.release-tag')}; Get-ChildItem -Path $build -Filter *.sh -Recurse | ForEach-Object { $c = [System.IO.File]::ReadAllText($_.FullName) -replace \"`r\", ''; [System.IO.File]::WriteAllText($_.FullName, $c, (New-Object System.Text.UTF8Encoding $false)) }; $bp = Join-Path $build 'bin\php'; if (Test-Path $bp) { $c = [System.IO.File]::ReadAllText($bp) -replace \"`r\", ''; [System.IO.File]::WriteAllText($bp, $c, (New-Object System.Text.UTF8Encoding $false)) }; $tar=$dist+'\pencarimovie-server.tar.gz'; tar -czf $tar -C ($dist+'\.build-tmp') 'pencarimovie-server'; Remove-Item -Recurse -Force $build"
